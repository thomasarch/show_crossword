class WeaveEightController < ApplicationController
  def show
  	now = Time.now
		system 'clear'
		...
		file0 = File.join(File.dirname(__FILE__), "../../lib/eight_deep_followers.yml")
		...
		def search_block(level)
			...
			while @list[level].count > 0	
				unless level == 7
					...
					set_depth(level)			
					if @list[level + 1].count > 0 && level < 7
	...
	file_name = File.join(File.dirname(__FILE__), "../../lib/answers/eight_letter_answers/#{root_letter}.yml")
